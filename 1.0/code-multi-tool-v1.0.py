...
# 抓取页面所有课程列表（多次点击查询，精确解析 td.jxbmc 关键词）
...
# 自动选课逻辑（多次点击查询，严格匹配 <td class="jxbmc"> 关键词）
        ...
        for rank, target_course in enumerate(target_courses, start=1):
            ...
            try:
                ...
                if target_row is None:
                    reason = f"未找到符合 <td class='jxbmc'> 关键词【{course_keyword}】的课程"
                    print_log(f"  ↳ 原因: {reason}")

                    cont = ask_user("提示", f"当前关键词【{course_keyword}】未找到符合课程，是否继续轮询？")
                    if cont:
                        print_log("  ↳ 用户选择继续寻找列表下一课程")
                        continue
                    else:
                        print_log("  ↳ 用户选择否，退出抢课")
                        return False

                tx_btn = target_row.locator("button:has-text('退选'), a:has-text('退选')").first
                xk_btn = target_row.locator("button[id^='btn-xk-'], button:has-text('选课')").first
...
class UnifiedCampusApp:
    def __init__(self, root):
        ...
        self.btn_fetch.pack(side="left", padx=(5, 5))

        # 清除缓存按钮
        self.btn_clear_cache = tk.Button(
            btn_fetch_frame, text="清除缓存", bg="#6C757D", fg="white", font=(SYS_FONT, 9, "bold"),
            command=self.clear_cache_action
        )
        self.btn_clear_cache.pack(side="left", padx=(0, 5))

        tk.Label(btn_fetch_frame, text="(预览时抓取，勾选后开选时点击开始)", font=(SYS_FONT, 8), fg="#666666").pack(side="left", padx=5)
        ...
        self.chk_browser = tk.Checkbutton(
            opt_frame, text="显示浏览器操作画面 (推荐开启)", variable=self.show_browser_var, font=(SYS_FONT, 9)
        )
    ...
    def start_fetch_courses(self):
        ...
        def _fetch_thread():
            asyncio.run(self._run_fetch_async(selected_browser, username, password, show_browser))

        threading.Thread(target=_fetch_thread, daemon=True).start()

    async def _run_fetch_async(self, selected_browser, username, password, show_browser):
        def print_log(msg):
        ...
        try:
            async with async_playwright() as p:
                ...
                courses = await fetch_courses_hlu(page, username, password, {}, print_log)
                await browser.close()
    ...
    async def execute_playwright(self, handler_func, selected_browser, username, password, kwargs, show_browser):
        ...
        try:
            async with async_playwright() as p:
                ...
                if res:
                    self.root.after(0, lambda: messagebox.showinfo("完成", "已完成"))
